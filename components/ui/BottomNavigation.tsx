...
export function BottomNavigation() {
  ...
  return (
    <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full max-w-md z-50">
      <div className="flex justify-around py-2 bg-transparent px-2">
        {navItems.map((item) => {
          const isActive = pathname === item.href || (item.href !== "/" && pathname.startsWith(item.href))
          return (
            <Link href={item.href} key={item.href} className="flex flex-col items-center gap-1 flex-1">
              <span
                ...
                `}
              >
                <item.icon className={`${isActive ? item.color : "text-gray-400"}`} size={20} />
              </span>
              <span className={`text-xs transition-all duration-200 text-center ${isActive ? item.text : "text-gray-400"}`}>{item.label}</span>
            </Link>
          )
        })}
      </div>
